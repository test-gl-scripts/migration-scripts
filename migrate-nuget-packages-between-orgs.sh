...
packages=$(GH_HOST="$SOURCE_HOST" GH_TOKEN=$GH_SOURCE_PAT gh api --paginate "orgs/$SOURCE_ORG/packages?package_type=nuget" -q '.[] | .name + " " + .repository.name')
...
echo "$packages" | while IFS= read -r response; do
  ...
  do
    ...
    echo $url
    curl -Ls -H "Authorization: token $GH_SOURCE_PAT" $url --output "${packageName}-${version}.nupkg" -s

    # must do this otherwise there is errors (multiple of each file)
    zip -d "${packageName}-${version}.nupkg" "_rels/.rels" "\[Content_Types\].xml" # there seemed to be duplicate of these files in the nupkg that led to errors in gpr
    dotnet nuget push $packageName-$version.nupkg --source "https://nuget.pkg.github.com/$TARGET_ORG/index.json" --api-key $GH_TARGET_PAT --skip-duplicate
